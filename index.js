...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=557798076858:+55 (77) 9807-6858\n' // ID do WhatsApp + número de telefone
            + 'END:VCARD'
/******END OF VCARD INPUT******/
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `� Ola @${num.split('@')[0]}\nSeja muito bem Vindo(a)!!! *${mdata.subject}*`
				...
				teks = `Demoro de sair  @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerB: '[❗] �Este comando so pode ser usado pelo Max! ',
			...
			const ownerNumber = ["557798076858@s.whatstapp.net"] // replace this with your number
